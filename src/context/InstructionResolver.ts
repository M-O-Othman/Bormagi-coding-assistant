...
function loadLayer(absolutePath: string, role: InstructionLayer['role']): InstructionLayer {
 ...
 */
export function resolveInstructions(workspaceRoot: string): EffectiveInstructions {
  const instrDir = path.join(workspaceRoot, INSTRUCTION_DIR);

  const globalLayer = loadLayer(path.join(instrDir, GLOBAL_INSTRUCTION), 'global');
  const repoLayer   = loadLayer(path.join(instrDir, REPO_INSTRUCTION),   'repo');

  const layers: InstructionLayer[] = [globalLayer, repoLayer];
